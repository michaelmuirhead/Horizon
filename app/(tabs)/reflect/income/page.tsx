...
import { incomeBreakdownByPayee } from "@/lib/reflect";
...
function IncomeBreakdown() {
  ...
  const now = new Date();
  const { months, entries } = incomeBreakdownByPayee(
    budgetTxs,
    now.getFullYear(),
    now.getMonth(),
    monthsBack,
  ...
  return (
    <>
      <SubpageHeader title="Income Breakdown" backHref="/reflect" />
      <div className="px-4 pt-2 pb-10">
        ...
        <div className="mt-4 rounded-2xl bg-card p-5 text-center">
          ...
          <p className="mt-1 text-xs text-fg/55">
            {formatCurrency(total / monthsBack)}/mo avg over {monthsBack}{" "}
            months
          </p>
        </div>
        ...
        {entries.length === 0 ? (
          <div className="mt-6 flex flex-col items-center gap-3 text-center text-fg/60">
            <p className="text-base">
              No income recorded in the last {monthsBack} months.
            </p>
            <Link
              href="/spending/new"
                  ...
                  >
                    {e.byMonth.map((v, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="self-end rounded-sm bg-emerald-400"
                          style={{
                            height: `${h}px`,
